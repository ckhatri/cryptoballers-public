window.App = {
  ...
  // our smart contract address
    ...
    BallerToken.totalPlayerSupply(function(error, results) {
    	var count = results['c'][0];
      count = 5
      console.log(count)
    	var playerCardHTML = ['<div class=\"card-deck\" style=\"padding:10%;margin:0px;\">'];
        ...
        for (var a = 0; a < count; a += 3) {
            ...
            playerCardHTML = playerCardHTML.concat(rowHTML);
        };
    	playerCardHTML = playerCardHTML.concat(['</div>']);
    	$('.teamCards').append($.makeArray(playerCardHTML).join(''));
    	for (var a = 0; a < 5; a++) {
    		$('.teamCards').find('#' + a).find('h4')[0].innerText = playerNames[a];
    		BallerToken.getPlayer(a, function(error, results) {
    			if (!error) {
            ...
            $('.teamCards').find('#' + index).find(':button').click(function() {
              BallerToken.purchasePlayer.sendTransaction(index, {from: web3.eth.accounts[0], value: results[1].toNumber()}, function(error, resultOfPurchase) {
      					if (error) {
